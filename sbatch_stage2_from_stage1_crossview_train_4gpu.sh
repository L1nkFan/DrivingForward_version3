#!/bin/bash
#SBATCH --job-name=df_v3_s2_cv_4g
...
# Cluster policy: <= 7 CPU cores per GPU
...
RUNTIME_CFG="configs/nuscenes/.phase2_runtime_stage1_crossview_train_4gpu.yaml"
...
sed -i "s#^\([[:space:]]*log_dir:[[:space:]]*\).*#\1'./results_stage2_from_stage1_crossview_4gpu_v1/'#g" "$RUNTIME_CFG"
sed -i "s#^\([[:space:]]*save_path:[[:space:]]*\).*#\1'./results_stage2_from_stage1_crossview_4gpu_v1/images'#g" "$RUNTIME_CFG"
sed -i "s#^\([[:space:]]*world_size:[[:space:]]*\).*#\14#g" "$RUNTIME_CFG"
sed -i "s#^\([[:space:]]*gpus:[[:space:]]*\).*#\1[0, 1, 2, 3]#g" "$RUNTIME_CFG"
sed -i "s#^\([[:space:]]*num_epochs:[[:space:]]*\).*#\16#g" "$RUNTIME_CFG"
