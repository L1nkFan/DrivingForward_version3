...
class NuScenesdataset(Dataset):
    ...
    def generate_depth_map(self, sample, sensor, cam_sample):
        ...
        # load and return if exists (with error handling)
        if os.path.exists(filename):
            try:
                ...
                if 'depth' in data:
                    return data['depth']
            except (zlib.error, IOError, KeyError, EOFError, 
                    pickle.UnpicklingError, _pickle.UnpicklingError,
                    ValueError, RuntimeError) as e:
                # 文件损坏或无法读取，删除并重新生成
                print(f"Warning: Corrupted depth file {filename}, removing and regenerating. Error: {e}")
                try:
                    os.remove(filename)
                except OSError:
                    pass
