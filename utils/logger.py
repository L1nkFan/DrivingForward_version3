...
class Logger:
    ...
    def log_tb(self, mode, inputs, outputs, losses, step):
        ...
        for l, v in losses.items():
            # Only scalar values are valid for add_scalar; skip metadata like strings.
            if isinstance(v, torch.Tensor):
                if v.numel() == 1:
                    writer.add_scalar(f'{l}', v.item(), step)
            elif isinstance(v, (int, float, np.number)):
                writer.add_scalar(f'{l}', float(v), step)
        ...
        for cam_id in range(self.num_cams):
            ...
            # Optional keys for both stage-1 and stage-2 outputs.
            if ('reproj_loss', scale) in target_view:
                plot_tb(writer, step, target_view[('reproj_loss', scale)], set_tb_title('cam', cam_id, 'reproj')) # reprojection image
            if ('reproj_mask', scale) in target_view:
                plot_tb(writer, step, target_view[('reproj_mask', scale)], set_tb_title('cam', cam_id, 'reproj_mask')) # reprojection mask
            plot_tb(writer,  step, inputs['mask'][:, cam_id, ...], set_tb_title('cam', cam_id, 'self_occ_mask'))
    
            if self.spatio:
                # Optional overlap visualizations for spatio mode.
                if ('overlap', 0, scale) in target_view:
                    plot_norm_tb(writer, step, target_view[('overlap', 0, scale)], set_tb_title('cam', cam_id, 'sp'))
                if ('overlap_mask', 0, scale) in target_view:
            ...
            if self.gaussian:
                ...
                # Optional gaussian-color visualization for gaussian mode.
                if ('gaussian_color', frame_id, scale) in target_view:
    ...
    def log_scalar(self, tag, value, step):
        """Log a validation scalar to TensorBoard."""
        # Args:
        #     tag: scalar name
        #     value: scalar value
        #     step: global step
        ...
        if hasattr(self, 'writers') and 'val' in self.writers:
            self.writers['val'].add_scalar(tag, value, step)
